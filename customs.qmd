...
comb_data <- feature1 %>%  mutate(name = case_when(
  ...
  name == "Generating plots by coding" ~ "Coding"
    ...
    mutate(gr = case_when(
      ...
      grepl("\\b(Coding|Customizable|Reproducable|Publicable|Inset)\\b", name) == TRUE ~ "Favored"))

test <-  comb_data %>% group_by(name, value) %>% dplyr::summarise(n = n()) %>% 
    mutate(gr = case_when(
      grepl("\\b(Speed|Zooming|GUI|Interactive|Inset)\\b", name) == TRUE ~ "Moderate",
      grepl("\\b(Coding|Customizable|Reproducable|Publicable)\\b", name) == TRUE ~ "Favored"))  %>% 
...
cbPalette1 <- c("#e3342f", "#6574cd", "#f6993f",  "#38c172" )

cbPalette2 <- c("white", "#3490dc","#f1a20b", "#9561e2", "#4dc0b5", "#f66d9b")
...
ggarrange(test1, test2, align = 'v', nrow = 2) 
```
...
bl2 <- table(abs(just[2] %>% na.omit() %>% rbind(data[77] %>% dplyr::rename(num = !!names(.)[1]) , .))) %>% data.frame() %>%  dplyr::rename(baseline = !!names(.)[1]) %>% 
 # mutate(baseline = as.numeric(baseline) * -1) %>% 
  mutate(baseline = ifelse(Freq > 2, paste0("-", as.character(baseline)), "Other")) %>%
  group_by(baseline) %>% summarise(Freq = sum(Freq)) %>% 
