...
data %>% select(23, 24) %>% 
  ...
  geom_histogram(bins=300, position="stack") + labs(x ="Number of channels") +
  theme_classic()
```
...
data %>% select(23, 24) %>% 
  ...
  gather(key=Type, value=Value) %>% 
  ggplot(aes(x=Value, fill=Type)) + 
  geom_histogram(bins=100, position="dodge") + labs(x ="Number of channels") +
  theme_classic()
```
...
data %>% select(23, 24) %>% 
  rename_at(vars(colnames(.)), ~ c("measure", "analyse")) %>% 
  filter(analyse < 500) %>% ggplot(., aes(measure, analyse)) + geom_point() + theme_classic() +
