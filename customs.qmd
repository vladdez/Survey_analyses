...
cv %>% 
  ggplot(aes(x=rate)) + 
  geom_histogram(bins=300) + labs(x ="analyse / measure") +
  theme_classic()+
  labs(caption = sprintf("N of respondents - %d",  nrow(cv))) +
  theme(legend.position="none", plot.caption = element_text(hjust=0), axis.text = element_text(size = 10))
```
...
data %>% select(23, 24) %>% 
  rename_at(vars(colnames(.)), ~ c("measure", "analyse")) %>% 
  filter(measure < 10000, analyse < 500)  %>% ggplot(., aes(measure, analyse)) + geom_point() + theme_classic() +
  scale_x_continuous(breaks = seq(0, 300, by = 20)) + scale_y_continuous(breaks = seq(0, 300, by = 20)) +
  labs(caption = sprintf("N of respondents - %d",  nrow(cv))) +
  theme(legend.position="none", plot.caption = element_text(hjust=0), axis.text = element_text(size = 10))
...
```{r}
bl <- table(abs(data[77])) %>% data.frame() %>%  dplyr::rename(baseline = !!names(.)[1]) 

bl %>%
    ggplot(data = ., aes(x = baseline, y= Freq)) +
    geom_bar(stat="identity") + labs(x = "Recomended baseline in msec", y = "Scores") +  
  scale_y_continuous(breaks=seq(0, 60, 5)) + theme_classic() +
  theme(legend.position="none", plot.title = element_text(hjust = 0.5)) +
  geom_text(aes(label = Freq, group = baseline), position = position_dodge(width = .9), vjust = -0.3)  +
