...
comb_data <- feature1 %>%  mutate(name = case_when(
  ...
  name == "Generating plots by coding" ~ "Coding"
    ...
    mutate(gr = case_when(
      ...
      grepl("\\b(Coding|Flexible|Reproducable|Publicable|Inset)\\b", name) == TRUE ~ "Favored"))
...
test  %>% 
  ...
  scale_color_manual(values=cbPalette) + theme_classic()  +  
...
test  %>% 
  ...
  scale_color_manual(values=cbPalette) + theme_classic()  + 
  labs(x = "Values on the Likert scale", y = "Scores") +  
  theme(
  ...
  labs(
    title = "When using an EEG, MEG, or iEEG visualization tool\nwhich features are important to you?"
  )
```
...
pol_not_plot2 <- as %>% 
  ...
  labs(x = "", y = "", fill ="Positive:", title = "In ERP plot, should positive voltages\nbe plotted upwards, or downwards?") +
  theme_classic()+
...
tti_data <- data[95] %>% rename_at(vars(colnames(.)), ~ c("toposeries_int")) %>% filter(!is.na(.), toposeries_int != "Other") %>%
  group_by(toposeries_int) %>% summarise(n = n()) %>% mutate(toposeries_int = case_when(
  toposeries_int == "The instantaneous time slices (single time points)" ~ "Time point",
  toposeries_int == "The mean value of time bin, centered around the labelled time (average over multiple time points)"~ "Average\nover\ntime points"
...
tti <-  tti_data %>% ggplot(., aes(y = toposeries_int, x = p)) + 
  ...
  labs(title = "How do you interpret time\non topoplot timeseries?", subtitle = "", y = "")+
...
ebd_fig <- ebd %>% 
    ggplot(., aes(y = position, x = percent_score)) + #, fill = as.factor(percent_score))) +
    geom_bar(position = "dodge", stat = "identity", width=0.5, fill ="#6BAED6") + theme_classic() +
   theme(axis.title.x=element_blank(), axis.text.x = element_blank(), text = element_text(family = "Lato"),
        legend.position="none", 
  ...
  ggtitle("In your figure, what\ndid the error bars represent?")+
    ...
    theme(plot.caption = element_text(hjust=0), axis.text = element_text(size = 10)) + xlim(0, 85) 
