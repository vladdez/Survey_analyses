...
```{r}
cbPalette2 <- c("#e3342f", "#38c172", "grey10", "#3490dc", "#f6993f", rep("grey10", 9) )
ggarrange(chart + labs(title="Which EEG, MEG, or iEEG\nanalysis platform\nhave you used for ERP plotting?")+
            theme( axis.text.y = element_text(color = rev(cbPalette2)),
          ...
          cit_plot + labs(title = "Citations per year for\nthe most popular EEG tools") +
            theme(plot.title = element_text(color = "grey10", size = 14, margin = margin(t = 15)))+
            scale_x_continuous(limits = c(2003.8, 2027), breaks = seq(2004, 2023, by = 3)), 
