...
n2 <- word_preproc(data, 80)  %>%
  mutate(type = case_when(
      ...
      grepl("\\b(channel|single|singleplot|individual)\\b", words)  == TRUE ~ "single or individual channel erp (plot)",
      grepl("\\b(channels|multichannel|multeple|multiple|multi|all|muti|electrodes)\\b", words)  == TRUE ~ "multichannel/electrode erp (plot)",
...
create_table <- function(t){
  tmp <- t[[1]] %>% mutate(total = sum(n)) %>% slice(1:3) %>% mutate(n1, type = plot_names[1]) %>% rename_with(.cols = 1, ~"names") %>% relocate(type, names )
  
  for (i in 2:8){
    tmp <- t[[i]]  %>% mutate(total = sum(n)) %>% slice(1:3) %>% 
      mutate(n1, type = plot_names[i]) %>% rename_with(.cols = 1, ~"names") %>% relocate(type, names) %>% 
      rbind(tmp, .)
  }
  tmp <- tmp %>% 
    group_by( type) %>%
...
mins <- image_read_pdf("data/eegvis_icons.pdf", density = 400)
mins[1]
```
