...
n1 <- word_preproc(data, 69)  %>% #View()
   mutate(type = case_when(
      ...
      grepl("\\b(average(d)?|averagded)\\b.*\\b(erp(s)?|ep|related)\\b|\\b(erp(s)?|ep|related)\\b.*\\baverage(d)?\\b", words) == TRUE ~ "averaged erp",
...
n5 <- word_preproc(data, 97)   %>%
  ...
  dplyr::select(type) %>%  table(.) %>%
  data.frame(.) %>% arrange(desc(Freq)) %>% filter(type != "bad")  %>%
  rename_at(vars(colnames(.)), ~ c("Topo grid", "n"))
n5
...
```{r}
plot_names <- c("ERP plot", "Butterfly plot", "Topoplot", "Topo array", "Topo grid", "ERP image", "Channel image", "Parallel plot")
