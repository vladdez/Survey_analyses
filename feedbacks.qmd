...
plot_names <- c("erp", "butterfly", "topo", "topo_array", "topo_grid", "erp_img", "parallel", "channel_img")
...
n2 <- word_preproc(data, 80)  %>%
  mutate(type = case_when(
      ...
      grepl("\\b(channel|single|singleplot|individual)\\b", words)  == TRUE ~ "single or individual channel erp",
      grepl("\\b(channels|multichannel|multeple|multiple|multi|all|muti|electrodes)\\b", words)  == TRUE ~ "multichannel/electrode erp",
...
n4 <- word_preproc(data, 90)  %>%
  mutate(type = case_when(
      ...
      grepl("\\btopo(plot|plots)?\\b.*\\btime(course|series)?\\b|\\btime(course|series)?\\b.*\\btopo(plot|plots)?|timetopoplot\\b", words_prep) == TRUE ~ "topo(plot) over time",
      grepl("\\btopo(plot|plots)?\\b.*\\bseries\\b|\\bseries\\b.*\\btopo(plot|plots)?\\b", words) == TRUE ~ "topo series",
      grepl("\\b(topo(s)?|topoplot(s)?)\\b", words_prep) == TRUE ~ "topo(plot)", 
 ...
 # mutate(type = ifelse(!grepl("topo(plot) over time", type) & grepl("\\b(topo(s)?|topoplot(s)?)\\b", words_prep), "topo(plot)", type)) %>% 
  #View()
  dplyr::select(type) %>%  table(.) %>% 
