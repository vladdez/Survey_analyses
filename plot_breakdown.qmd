...
struggle1 %>% filter( parts != "-") %>% dplyr::select(parts) %>% 
  ...
  mutate(problems = case_when(
      ...
      grepl("\\b(axes|lable(s)?|label(s)?|legend|ticks)\\b", parts) == TRUE ~ "plot components (axes, lables, legend)", 
      ...
      )) %>% filter(nchar(parts) != 0) %>% filter(!is.na(problems))  %>% group_by(problems) %>% dplyr::summarise(n = n()) %>% mutate(percent = round(n / sum(n), 2) * 100) %>% arrange(desc(n)) %>% 
